...
# Camera Upgrade."
# Must name the update itself, so ARRI's "MENU > Media > Prepare USB
...
def bench_line(cam):
    ...
    # A slot is only quoted when the manufacturer names one. RED bodies
    # have a single media bay and never do, so that step is left out
    # rather than invented.
    if not (file_name and menu):
        return None

    folder = FOLDER_RE.search(joined)
    if folder:
